...
def regress(bold_file, brain_mask_file, confounds_file, csf_mask, FD_file, conf_list, TR, lowpass, highpass, smoothing_filter,
    ...
    if run_aroma:
        ...
        smooth_path=os.path.abspath(cwd+'/%s_smoothed.nii.gz' % (filename_split[0]))
...
class data_diagnosisInputSpec(BaseInterfaceInputSpec):
    bold_file = File(exists=True, mandatory=True, desc='input BOLD time series')
    cleaned_path = File(exists=True, mandatory=True, desc='ref file to realignment time series')
    brain_mask_file = File(exists=True, mandatory=True, desc='ref file to realignment time series')
...
class data_diagnosis(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        mel_out=os.path.abspath('melodic.ica/')
        os.mkdir(mel_out)
        command='melodic -i %s -o %s -m %s --report' % (self.inputs.cleaned_path, mel_out, self.inputs.brain_mask_file)
        os.system(command)
...
def seed_based_FC(bold_file, brain_mask, seed):
    ...
    os.system('antsApplyTransforms -i %s -r %s -o %s -n GenericLabel' % (seed, brain_mask, os.path.abspath(os.path.basename(seed))))

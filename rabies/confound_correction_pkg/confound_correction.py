...
def clean_image(input_bold, brain_mask, FD_csv, motion_params_csv, # necessary input files
    ...
    """
    Core function for carying confound correction of an image using the set of RABIES tools.

    Parameters
    ----------
    input_bold : filepath or sitk.Image
        The input 4D timeseries image to clean.

    brain_mask : filepath or sitk.Image
        A brain mask in the same space as input_bold

    FD_csv : filepath or pd.DataFrame
        The framewise displacement CSV generated by RABIES associated with input_bold.

    motion_params_csv : filepath or pd.DataFrame
        The motion parameters CSV generated by RABIES associated with input_bold.
                
    WM_mask : filepath or sitk.Image, default=None
        A WM mask in the same space as input_bold

    CSF_mask : filepath or sitk.Image, default=None
        A CSV mask in the same space as input_bold
    ...
    FD_censoring : bool, default=False
        Whether to apply FD censoring.
        
    FD_threshold : float, default=0.05
        The FD censoring threshold.
    ...
    ica_aroma_dim : int, default=0
        ICA dimensionality for AROMA. If 0, uses MELODIC's automated estimator.
    ...
    edge_cutoff : int, default=0
        Number of seconds of data to remove at each edge after frequency filtering to 
        remove edge artefacts.

    nuisance_regressors : list, default=[]
        List of nuisance regressors. Possible options are: "WM_signal", "CSF_signal", 
        "vascular_signal", "global_signal", "aCompCor_percent", "aCompCor_5", "mot_6", 
        "mot_24"

    generate_CR_null : bool, default=False
        Whether to estimate overfitting from confound regression by generating phase 
        randomised regressors, and comparing the variance explained by the real 
        regressors VS the randomized regressors.

    scale_variance_voxelwise : bool, default=False
        If scaling was not applied voxelwise with voxelwise_standardization or 
        voxelwise_mean, this option standardize the variance at each voxel to be equal, 
        while preserving to total variance of the 4D timeseries (i.e. voxels have same 
        variance, but not unit variance).
    ...
    slicewise_correction_direction : str, default='Off'
        ...
        independently along the provided slice direction. This strategy can be useful in 
        the presence of artefacts (e.g. intensity spikes) that are not shared across 
        slices due to time delays between slice acquisition, in which case a slicewise 
        nuisance regression may perform better. Otherwise, in the case of images with 
        spatial gaps between slices, smoothing should be applied slicewise using this 
        parameter. To apply slicewise correction, provide as input one of the RAS axial 
        direction for slice selection, i.e. this parameter takes as input one of: 'RL', 
        'AP' or 'SI'. For accurate slice selection, the axial directions of the input 
        nifti images must be properly encoded. If no direction is provided, standard 
        whole-brain correction is conducted.

    nipype_log : nipype.logging.getLogger('nipype.workflow'), default=None
        Provide a nipype log for printing statements.

    Returns
    -------

    timeseries_img : sitk.Image
        Output cleaned timeseries image.

    CR_data_dict : dict
        Output cleaned timeseries image.

    VE_spatial_map : sitk.Image
        R2 of nuisance regression voxelwise.

    STD_spatial_map : sitk.Image
        Voxelwise standard deviation of cleaned timeseries.

    CR_STD_spatial_map : sitk.Image
        Voxelwise standard deviation of fitted nuisance timeseries.

    random_CR_STD_spatial_map : sitk.Image
        Voxelwise standard deviation of fitted randomized regressors with 
    ...
    if (highpass is not None) or (lowpass is not None):
        ...
        timeseries, _ = spectral_interpolation(x=timeseries, time_step=TR, time_mask=frame_mask, rescale=True)
        del _
        motion_regressors_array, _ = spectral_interpolation(x=motion_regressors_array, time_step=TR, time_mask=frame_mask, rescale=True)
        del _

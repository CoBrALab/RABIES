...
def execute_workflow():
    ...
    try:
        ...
        workflow.run(plugin=plugin, plugin_args={'max_jobs': 50, 'dont_resubmit_completed_jobs': True, 'n_procs': int(
            os.environ["local_threads"]), 'qsub_args': '-pe smp %s' % (os.environ["min_proc"])})
...
def preprocess(opts, cr_opts, analysis_opts):
    ...
    os.environ["template_anat"] = convert_to_RAS(
        str(opts.anat_template), os.environ["RABIES"]+'/template_files')

    if not os.path.isfile(str(opts.brain_mask)):
        raise ValueError("--brain_mask file doesn't exists.")
    os.environ["template_mask"] = convert_to_RAS(
        str(opts.brain_mask), os.environ["RABIES"]+'/template_files')

    if not os.path.isfile(str(opts.WM_mask)):
        raise ValueError("--WM_mask file doesn't exists.")
    os.environ["WM_mask"] = convert_to_RAS(
        str(opts.WM_mask), os.environ["RABIES"]+'/template_files')

    if not os.path.isfile(str(opts.CSF_mask)):
        raise ValueError("--CSF_mask file doesn't exists.")
    os.environ["CSF_mask"] = convert_to_RAS(
        str(opts.CSF_mask), os.environ["RABIES"]+'/template_files')

    if not os.path.isfile(str(opts.vascular_mask)):
        raise ValueError("--vascular_mask file doesn't exists.")
    os.environ["vascular_mask"] = convert_to_RAS(
        str(opts.vascular_mask), os.environ["RABIES"]+'/template_files')

    if not os.path.isfile(str(opts.labels)):
        raise ValueError("--labels file doesn't exists.")
    os.environ["atlas_labels"] = convert_to_RAS(
        str(opts.labels), os.environ["RABIES"]+'/template_files')

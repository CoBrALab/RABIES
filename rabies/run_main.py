...
import rabies
rabies_path = os.path.dirname(os.path.realpath(rabies.__file__))
...
def get_parser():
    ...
    g_atlas.add_argument('--brain_mask', action='store', type=Path,
                         default="%s/template_files/DSURQE_40micron_mask.nii.gz" % (
                             rabies_path),
                         help='Brain mask for the template.')
    g_atlas.add_argument('--WM_mask', action='store', type=Path,
                         default="%s/template_files/DSURQE_40micron_eroded_WM_mask.nii.gz" % (
                             rabies_path),
                         help='White matter mask for the template.')
    g_atlas.add_argument('--CSF_mask', action='store', type=Path,
                         default="%s/template_files/DSURQE_40micron_eroded_CSF_mask.nii.gz" % (
                             rabies_path),
...
def install_DSURQE(log):
    out_dir = rabies_path+'/template_files'
    install=False
    # verifies whether default template files are installed and installs them otherwise
    if not os.path.isfile("%s/DSURQE_40micron_average.nii.gz" % (out_dir)):
        install=True
    elif not os.path.isfile("%s/DSURQE_40micron_mask.nii.gz" % (out_dir)):
        install=True
    elif not os.path.isfile("%s/DSURQE_40micron_eroded_WM_mask.nii.gz" % (out_dir)):
        install=True
    elif not os.path.isfile("%s/DSURQE_40micron_eroded_CSF_mask.nii.gz" % (out_dir)):
        install=True
    elif not os.path.isfile("%s/DSURQE_40micron_labels.nii.gz" % (out_dir)):
        install=True
    elif not os.path.isfile("%s/vascular_mask.nii.gz" % (out_dir)):
        install=True
    elif not os.path.isfile("%s/melodic_IC.nii.gz" % (out_dir)):
        install=True
    if install:
        from rabies.preprocess_pkg.utils import run_command
        log.info("SOME FILES FROM THE DEFAULT TEMPLATE ARE MISSING. THEY WILL BE INSTALLED BEFORE FURTHER PROCESSING.")
        rc = run_command('install_DSURQE.sh %s' % (out_dir), verbose = True)

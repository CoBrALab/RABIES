...
def init_main_postcommonspace_wf(data_csv, data_dir_path, output_folder, csv_labels, tr, tpattern, apply_STC=True, bold_preproc_only=False, commonspace_transform=False,compute_WM_CSF_masks=False,
    ...
    if bold_preproc_only:
        #will be using the template files as anatomical substitutes
        print("BOLD preproc only!")
        anat_nii = os.environ["template_anat"]
        labels = os.environ["atlas_labels"]
        mask = os.environ["template_mask"]
    ...
    if bold_preproc_only:
        ...
        workflow.connect([
            ...
            (commonspace_selectfiles, file_info, [("bold_file_run1", "file_path")]),

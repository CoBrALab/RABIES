...
def prep_bids_iter(layout, bids_filter, bold_only=False, inclusion_list=['all'], exclusion_list=['none']):
    ...
    return split_name, scan_info, run_iter,echo_iter, structural_scan_list, number_functional_scans
...
class BIDSDataGraberInputSpec(BaseInterfaceInputSpec):
    ...
    bids_filter = traits.Dict(exists=True, mandatory=True,
                              desc="BIDS specifications for file selection")
    scan_info = traits.Dict(exists=True, mandatory=True,
                            desc="Information required to find the scan")
    run = traits.Any(desc="Run number")
    echo = traits.Any(desc="Echo number")
...
class BIDSDataGraberOutputSpec(TraitedSpec):
    out_file = traits.Either(traits.List(File(exists=True)),
                             File(exists=True),
                             desc="Selected file(s) based on the provided parameters.")
...
class BIDSDataGraber(BaseInterface):
    """
    This interface selects a scan from the BIDS directory based on the
    input specifications. It can handle both single-echo and multi-echo data.
    """
    ...
    def _run_interface(self, runtime):
        subject_id = self.inputs.scan_info['subject_id']
        session = self.inputs.scan_info.get('session', None)
        run = self.inputs.scan_info.get('run', self.inputs.run)
        echo = self.inputs.scan_info.get('echo', self.inputs.echo)

        bids_filter = self.inputs.bids_filter.copy()
        bids_filter['subject'] = subject_id
        if session is not None:
            bids_filter['session'] = session
        if run is not None:
            bids_filter['run'] = run
        if echo is not None:
            bids_filter['echo'] = echo

        layout = BIDSLayout(self.inputs.bids_dir, validate=False)
        try:
            file_list = layout.get(extension=['nii', 'nii.gz'], **bids_filter)
            if not file_list:
                raise ValueError(f"No files found matching BIDS filter: {bids_filter}")
            if len(file_list) > 1:
                raise ValueError(f"Multiple files found matching BIDS filter: {bids_filter}")
            file_path = file_list[0].path
        except Exception as e:
            raise ValueError(f"Error with BIDS filter {bids_filter}: {e}")

        setattr(self, 'out_file', file_path)

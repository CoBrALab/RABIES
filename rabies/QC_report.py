...
class PlotOverlapInputSpec(BaseInterfaceInputSpec):
    moving = File(exists=True, mandatory=True, desc="Moving image from registration.")
    fixed = File(exists=True, mandatory=True, desc="Fixed image from registration.")
    reg_name = traits.Str(mandatory=True, desc="Name of the registration which is displayed.")
...
class PlotOverlap(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        if not 'resampled_template.nii.gz'==os.path.basename(self.inputs.fixed):
            subject_id='sub-'+(os.path.basename(self.inputs.moving).split('_ses-')[0]).split('sub-')[1]
...
class PlotMotionTraceInputSpec(BaseInterfaceInputSpec):
    confounds_csv = File(exists=True, mandatory=True, desc="Confound csv file with motion timecourses.")
    out_dir = traits.Str(mandatory=True, desc="Directory for QC outputs.")
...
class PlotMotionTrace(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        import pandas as pd
        import os
        subject_id=os.path.basename(self.inputs.confounds_csv).split('_ses-')[0]
        session=os.path.basename(self.inputs.confounds_csv).split('_ses-')[1][0]
